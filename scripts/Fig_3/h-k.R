...
pdf(file = 'h-k.pdf', width = 14, height = 7.5)
...
lng_liv$cell_type[lng_liv$cell_type %in% 'Bronchiolar and alveolar epithelial cells'] = 'bronchoalveolar cells'

p_ =  VlnPlot(lng_liv, features = 'liv_score', group.by = "cell_type", pt.size = 0, combine = FALSE, log = F, sort = F, cols = c('blue','red'))[[1]]+
      theme(legend.position = 'none', axis.text.x = element_text(angle = -45), text = element_text(face = 'bold'))+
      labs(title = 'PDAC-liver signature', y = 'Signature score', x = NULL)+
      stat_summary(fun = mean, geom ='point', size = 19, colour = "black", shape = 95)

q_ =  VlnPlot(lng_liv, features = 'lng_score', group.by = "cell_type", pt.size = 0, combine = FALSE, log = F, sort = F, cols = c('blue','red'))[[1]]+
      theme(legend.position = 'none',  axis.ticks.y = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(angle = -45),
...
set.seed(42)
liv_cells = names(lng_liv$cell_type[lng_liv$Organ %in% 'Liver'])     # adult liver cells
lng_cells = names(lng_liv$cell_type[lng_liv$Organ %in% 'Lung'])      # adult lung cells
