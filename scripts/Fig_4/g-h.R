# Fig. 3j; This script computes single-sample enrichment scores (SSES) of PDAC recurrence signatures in MetMap500 cohort.
# MetMap500's cohort is bulk RNAseq data of cell lines from various cancer types including PDAC.
...
load(file = '../data/benchmark/single_cell/adult/merged_human_adult_lung_liver_processed.RData')

# reading in liver/lung met PDAC signatures

pdac_lung_markers = read.delim('../out/integration/cancer/treated_untreated/conditions/metastasis/DEG/cell_types_markers_main_subtypes_lung.tsv', header = T, sep = '\t')
...
pdac_liver_markers = read.delim('../out/integration/cancer/treated_untreated/conditions/metastasis/DEG/cell_types_markers_main_subtypes_liver.tsv', header = T, sep = '\t')
...
# tropism data
lung_tropism = read.delim(file = '../data/benchmark/bulk/MetMap500/lung_metastasis_potential.txt', sep = '\t', header = T)
rownames(lung_tropism) = lung_tropism$CCLE_name

liver_tropism = read.delim(file = '../data/benchmark/bulk/MetMap500/liver_metastasis_potential.txt', sep = '\t', header = T)
...
# meta data file
sampleMeta = read.delim(file = '../data/benchmark/bulk/MetMap500/MetMap_cell_line_annotation.txt', sep = '\t', header = T)
...
# expression matrix
exprsMat = t(read.delim(file = '../data/benchmark/bulk/MetMap500/CCLE_cell_lines_exprs_mat.csv', sep = ',', check.names = F, as.is = T, header = T, row.names = 1))
...
pdf(file = 'PDAC_recurrence_signature_MetMap500.pdf', width = 7.5, height = 7.5, fonts = 'Helvetica')
