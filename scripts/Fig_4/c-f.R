# This script takes adult cancer lung and liver seurat objects and checks the enrichment of PDAC recurrence signatures in them.
...
pdac_liver_markers = pdac_liver_markers[pdac_liver_markers$gene %in% rownames(lng_liv),]      # Att.: rownames(lng_liv)? Default assay is RNA
...
pdac_lung_markers = pdac_lung_markers[pdac_lung_markers$gene %in% rownames(lng_liv),]         # Att.: rownames(lng_liv)? Default assay is RNA
...
# DefaultAssay(lng_liv) = 'RNA'
# save(lng_liv, file = '../data/benchmark/single_cell/adult/merged_human_adult_cancer_lung_liver_processed.RData')

# Visualization ####
message('Visualization')

pdf(file = 'c-f.pdf', width = 7.5, height = 7.5)
...
p_ =  VlnPlot(lng_liv, features = 'liv_score', group.by = "histology", pt.size = 0, combine = FALSE, log = F, sort = F, cols = c('red','blue'))[[1]]+
      ...
      stat_summary(fun = median, geom ='point', size = 12, colour = "black", shape = 95)
...
for(c_ in list(liv_cells, lng_cells))
{
  ...
  cat('PDAC recurrence signatures in cells of (d) ', lng_liv$tissue[c_][1],': ',
  ...
  message('PDAC recurrence signatures in cells of (q) ', lng_liv$tissue[c_][1],': ',
